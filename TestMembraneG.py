...
if 0:
    ...
    write_external_DACs_neurodyn(dev, IinMaster_gmax, 0.9, 1)           
    write_external_DACs_neurodyn(dev, IinMasteralpha_beta, 0.9, 1)               
    ...
    write_external_DACs_neurodyn(dev, IinMaster_gmax, 0.6195, 2)         # dac = 0.722 - IMaster = 200.6nA; dac = 0.6195 - IMaster = 40nA   
    write_external_DACs_neurodyn(dev, IinMasteralpha_beta, 0.4405, 2)             # dac = 0.672 - IRef = 99.4 nA; dac = 0.4405 - IRef = 0.4nA 
    ...
    write_external_DACs_neurodyn(dev, IinMaster_gmax, 0.6195, 3)          
    write_external_DACs_neurodyn(dev, IinMasteralpha_beta, 0.4405, 3)            
    ...
    write_external_DACs_neurodyn(dev, IinMaster_gmax, 0.6255, 4)        # dac = 0.6255v - IMaster = 40nA 
    write_external_DACs_neurodyn(dev, IinMasteralpha_beta, 0.442, 4)             # dac =  0.442v - IRef = 0.4nA  
...
neurodyn_sel = 0                                                           # options - 0, 1, 2, 3
...
#write_external_DACs_neurodyn(dev, IinMaster_gmax, 0.7063, neurodyn_sel+1)          # IinMaster_gmax = 0.722 -- IMaster = 200.6nA, dac = 0.62 -- Imaster = 40.33nA, dac = 0.7575 -- Imaster = 400.2nA, dac = 1.2771 -- Imaster = 4.0004uA, dac = 0.7063 -- Imaster = 150nA
#write_external_DACs_neurodyn(dev, IinMasteralpha_beta, 0.502, neurodyn_sel+1)              # IinMasteralpha_beta = 0.672 -- IRef = 99.4 nA, dac = 0.443 -- Iref = 0.4nA, dac = 0.53 -- Iref = 4.08nA, dac = 0.622 -- Iref = 40.1nA, dac = 0.502 -- Iref = 2nA
...
write_external_DACs_neurodyn(dev, IinMaster_gmax, 0.525, neurodyn_sel+1)   # socket 1 - 0.6211v -- 40nA        
write_external_DACs_neurodyn(dev, IinMasteralpha_beta, 0.58, neurodyn_sel+1)       # socket 1 - 0.4422v -- 0.4nA       
...
neuron = 0 # moved this up, used for setting internal DACs in the end

NEURODYN_PARAMS = 0
#Gert's from email thread - NeuroDyn spiking (bursting?, HCO?) parameters
parms['biasgErev'][0][0] = [[gNa, ENa], [gK, 1023], [gL, EL]]                                     # Neuron 1 -- parms['biasgErev'][0][0][0][0] -- sodium maximal conductance, parms['biasgErev'][0][0][0][1] -- sodium maximal reversal potential, parms['biasgErev'][0][0][1][0] -- potassium maximal conductance, parms['biasgErev'][0][0][1][1] -- sodium reversal potential, parms['biasgErev'][0][0][2][0] -- leak maximal conductance, parms['biasgErev'][0][0][2][1] -- leak reverse potential 
parms['signgErev'][0][0] = [[1, 1], [1, -1], [1, 1]]                                                # sign bits for the same as described in line 274
parms['biasgErev'][1][0] = [[gNa, ENa], [gK, 1023], [gL, EL]]                                     # Neuron 2
parms['signgErev'][1][0] = [[1, 1], [1, -1], [1, 1]]
parms['biasgErev'][2][0] = [[gNa, ENa], [gK, 1023], [gL, EL]]                                     # Neuron 3
parms['signgErev'][2][0] = [[1, 1], [1, -1], [1, 1]]
parms['biasgErev'][3][0] = [[gNa, ENa], [gK, 1023], [gL, EL]]                                     # Neuron 4
...
#Gert's from email thread - NeuroDyn spiking (bursting?, HCO?) parameters
parms['biasAlphaBeta'][0][0][0][0][:] = [0, 0, 0, m_alpha, 0, 0, 0]           # m alpha
parms['biasAlphaBeta'][0][0][0][1][:] = [0, 0, 0, m_beta, 0, 0, 0]           # m beta
parms['biasAlphaBeta'][0][0][1][0][:] = [0, 0, 0, h_alpha, 0, 0, 0]            # h alpha
parms['biasAlphaBeta'][0][0][1][1][:] = [0, 0, 0, h_beta, 0, 0, 0]            # h beta
parms['biasAlphaBeta'][0][0][2][0][:] = [0, 0, 0, n_alpha, 0, 0, 0]            # n alpha
parms['biasAlphaBeta'][0][0][2][1][:] = [0, 0, 0, n_beta, 0, 0, 0]            # n beta
