...
VREF_nisoc      = 1.8             # NISoC DAC reference 
...
def write_DAC_nisoc(FPGA, addr, value):
    """
    functional description - writes Voltage DAC LTC2666.
    """
    if 0 <= addr <= 7:
...
def Data_Read_USB(FPGA=FPGA,filename='./Data/default.h5',recordingtime=8):
    ...
    # this line is creating an empty extendable array named "data" in the root group of an HDF5 file, 
    # with elements of type unsigned 8-bit integer and a shape that can dynamically grow in one 
    # dimension while having a fixed size (columns) in the other dimension.
